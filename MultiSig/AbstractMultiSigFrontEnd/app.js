...
function sendEther(){
    ...
    web3.eth.sendTransaction({from: web3.eth.coinbase, to: "0xF8bE6854B2cFf7F351898c4893F38C2a22A9D86B", value: web3.toWei(contributeAmount.value,'wei')},(err,res)=>{
        if(!err){
            console.log(res);
        }
    })
...
function submitProposalFunc(){
    ...
    contractAddress.submitProposal(web3.toWei(amt.value),(err,res)=>{
        if(!err){
            console.log(res);
...
function signerVoteFunc(){
    ...
    contractAddress.getSignerVote(signerAddr.value,benAddr.value,(err,res)=>{
        if(!err){
            if(res.toNumber() == 2){
            document.getElementById('signerVoterResult').innerHTML = 'He has voted Rejected';
...
function withdrawBenAmount(){
    ...
    contractAddress.withdraw(web3.toWei(val.value),(err,res)=>{
        if(!err){
            console.log(res);
